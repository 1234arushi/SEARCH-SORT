...
int search(int arr[], int n, int x, int k)
{
    ...
    while(i<n)
    {
        ...
        {
            int diff=abs(arr[i]-x);//how far the required element
            //can be from the current elemnet
            int jumps=diff/k;//how??
            i+=max(1,jumps);//why i+=?
        }
    }
    return -1;
    
    /*basic linear search:my method
    for(int i=0;i<n;i++)
    {
        if(arr[i]==x)
        {
